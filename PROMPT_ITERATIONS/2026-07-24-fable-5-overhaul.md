...
1. **Silent delivery failure on long-horizon runs** (`ses_076dbcccbf`, bbtws db). A 90-message `general` review agent finished cleanly (`finish: stop`) with zero `send_message` and zero `close_agent` — it wrote its review as final assistant text and ended, the exact "text-is-deliverable" drift iteration 9 legislated against, resurfacing deep into a long session. Matches Anthropic's documented fable-5 early-stopping mode ("deep into a long session... can occasionally end a turn with a text-only statement of intent"). A same-day boxbox incident (three research children, `ses_06dd00e78ffe` et al.) initially looked like the same class but audit traced it to a since-fixed proxy APIError (gemini model routing) — infra, not prose; the bbtws `cold_review` session is the clean prose-failure specimen. Fleet audit baseline: 43/45 recent bbtws subagent sessions were fully contract-compliant (`send:1 close:1 stop`), so the contract works; the residual failure pocket is long (~80+ message) review/analysis sessions.
...
- `agent.metric.safety_net_firing_rate` → should drop to ~0 (was the Discovery #1 signal). `deliverable_arrival_rate` → ~1. Caveat found during the fleet audit: the `event` table is empty on both boxbox and bbtws — the iteration-9 `agent.metric.*` events are bus-ephemeral and nothing persists them, so these rates currently have no historical data source. Follow-up candidate: persist `agent.metric.*` to the sourced event log (or a jsonl) so Observe is measurable rather than anecdotal.
