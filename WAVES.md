...
API pricing is `input_tokens * x + output_tokens * y`. In an agentic session, every tool call is a separate API request — and every request sends the entire conversation history as input. The first tool call sends the system prompt plus one message. The 50th tool call sends the system prompt plus 50 messages of accumulated context. Input cost per request grows linearly with session length.

A long session doing 80 tool calls doesn't just cost more at the end — it costs more on every single call after the first. The 80th call is paying for 79 messages of context it probably isn't using. Across the full session, total input token spend grows quadratically with the number of tool calls.

Waves cut this by resetting the accumulation. Four sessions of 20 tool calls each have far lower total input cost than one session of 80 tool calls, even though the same amount of work gets done. Each session starts with a small, fixed context (the wave files) instead of inheriting the full history of everything before it.
