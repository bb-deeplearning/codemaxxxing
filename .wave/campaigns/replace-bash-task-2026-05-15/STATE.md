...
| 4 | complete | ses_1d77620ccffe7kZkjEtVMFkHjJ | TBD | Drop `tool.shell` and `tool.task` from registry builtin; plugin hook bridge (legacy-then-new dispatch); 6 invariants + snapshot diff unskipped; 4 plugin contracts; bench best-by-p99 (-29% p99); GOTCHA addendum |
