...
**Residuals:** both closed 2026-07-21. (1) `startShell` now wraps the same `session-run:` key via `EffectFlock.tryAcquire` (fail-fast, no park) — a cross-process shell during a foreign lap dies `Session.BusyError`; pinned in effect-flock.test.ts + prompt.test.ts ("a shell from a second instance rejects..."). (2) bare session loop verbs (message/prompt_async/shell/command/abort/loop/summarize/revert/unrevert/fork/init, message DELETE, permission replies) route to the SESSION's own directory when no explicit `?directory=`/header rides the request — `isSessionHomeRoute` in server/shared/workspace-routing.ts, wired into BOTH backends' directory resolution (httpapi workspace-routing middleware + hono InstanceMiddleware); reads deliberately stay bare (routing a GET could spawn an instance). Pinned in test/server/workspace-routing.test.ts.
